...
> {
    ...
    const parsed$ = schemas$.safeParse(
        input$,
        (value$) => operations.UpdateTopicStatusRequest$outboundSchema.parse(value$),
    ...
    const context = { operationID: "updateTopicStatus", oAuth2Scopes: [], securitySource: null };
    ...
    const [result$] = await m$.match<
        ...
        | RequestTimeoutError
        | ConnectionError
    >(m$.json(200, operations.UpdateTopicStatusResponseBody$inboundSchema))(response);
    if (!result$.ok) {
        return result$;
    }
