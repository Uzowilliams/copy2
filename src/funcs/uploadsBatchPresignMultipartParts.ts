...
> {
    ...
    const parsed$ = schemas$.safeParse(
        input$,
        (value$) =>
            operations.BatchPresignMultipartPartsRequestBody$outboundSchema.optional().parse(
                value$
            ),
    ...
    const [result$] = await m$.match<
        ...
        | RequestTimeoutError
        | ConnectionError
    >(m$.json(200, operations.BatchPresignMultipartPartsResponseBody$inboundSchema))(response);
    if (!result$.ok) {
        return result$;
    }
