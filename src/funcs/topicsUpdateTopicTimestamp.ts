...
> {
    ...
    const parsed$ = schemas$.safeParse(
        input$,
        (value$) => operations.UpdateTopicTimestampRequest$outboundSchema.parse(value$),
    ...
    const context = { operationID: "updateTopicTimestamp", oAuth2Scopes: [], securitySource: null };
    ...
    const [result$] = await m$.match<
        ...
        | RequestTimeoutError
        | ConnectionError
    >(m$.json(200, operations.UpdateTopicTimestampResponseBody$inboundSchema))(response);
    if (!result$.ok) {
        return result$;
    }
